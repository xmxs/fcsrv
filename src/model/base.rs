...
impl ImageClassifierPredictor {
    ...
    #[inline]
    pub fn predict(&self, image: DynamicImage) -> Result<i32> {
        let predictions: Result<Vec<(f32, u32)>> = (0..6)
            .into_par_iter()
            .map(|i| {
                let ts = process_classifier_image(&image, i, (52, 52))?;
                let prediction = self.run_prediction(ts)?;
                let prediction_value = prediction[0];
                Ok((prediction_value, i))
            })
            .collect();
